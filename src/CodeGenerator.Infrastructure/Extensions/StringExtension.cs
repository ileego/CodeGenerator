...
namespace CodeGenerator.Infrastructure.Extensions
{
    public static class StringExtension
    {
        ...
        /// <summary>
        /// Change the string to pascal name
        /// </summary>
        /// <param name="input"></param>
        /// <param name="split"></param>
        /// <returns></returns>
        public static string ToPascal(this string input, char split = '_')
        {
            input = input.ToLower();
        ...
        /// <returns></returns>
        public static bool ContainsDbKeyword(this string value)
        {
            const string strKeyWord = @"select|insert|delete|from|count(|drop table|update|truncate|asc(|mid(|char(|xp_cmdshell|exec master|netlocalgroup administrators|:|net user|""|or|and|root|sa|dba";
        ...
        /// <param name="replaceValue">要替换的子串</param>
        /// <param name="newValue">新子串</param>
        /// <returns></returns>
        public static string Replace(this string value, string replaceValue, string newValue)
        {
            if (string.IsNullOrEmpty(value)) return string.Empty;
            value = Regex.Replace(value, replaceValue, newValue);
        ...
        public static string GetHMACMD5(this string source, string key)
        {
            ...
            var buff = keyBytes.ComputeHash(Encoding.UTF8.GetBytes(source));
        ...
        /// <returns></returns>
        // ReSharper disable once InconsistentNaming
        public static string GetSHA256(this string source)
        {
            SHA256 s256 = new SHA256Managed();
            var bytes = s256.ComputeHash(Encoding.Default.GetBytes(source));
            s256.Clear();
        ...
        public static MemoryStream GetQrCode(this string planText, QRCodeGenerator.ECCLevel eccLevel = QRCodeGenerator.ECCLevel.Q, int pixelsPerModule = 15)
        {
            ...
            {
                /*
                 * QRCodeGenerator.ECCLevel.L 7 % may be lost before recovery is not possible
                 * QRCodeGenerator.ECCLevel.M 15 % may be lost before recovery is not possible
                 * QRCodeGenerator.ECCLevel.Q 25 % may be lost before recovery is not possible
                 * QRCodeGenerator.ECCLevel.H 30 % may be lost before recovery is not possible
                 */
                using (var qrCodeData = generator.CreateQrCode(planText, eccLevel))
                {
            ...
            bitmap.Save(ms, ImageFormat.Png);//将此图像以Png图像文件的格式保存到流中  
            //回收资源  
            bitmap.Dispose();
            return ms;
        }
